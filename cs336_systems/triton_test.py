...
def weighted_sum_fwd(
    ...
    # offsets = tl.arange(0, D_TILE_SIZE) * (row_tile_idx + 1)
    # tl.device_print("rand", tl.rand(0, offsets))
    # print("rand1", row_tile_idx, tl.rand(0, row_tile_idx))
...
class WeightedSumFunc(torch.autograd.Function):
    ...
    def forward(ctx, x, weight):
        ...
        n_rows = y.numel()
        print("before triton")
        print(triton.cdiv(n_rows, ctx.ROWS_TILE_SIZE))
        print("triton.next_power_of_2(D)", triton.next_power_of_2(D))
        print(ctx.__dict__)
